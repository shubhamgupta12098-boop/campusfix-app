...
export function ComplaintDetailScreen({ complaintId, onBack }) {
    ...
    useEffect(() => {
        ...
        // Staff may upload Before/After evidence from another portal/tab.
        // Refresh this Admin detail immediately when shared local data changes.
        const refreshEvidence = () => void load({ quiet: true });
        const onStorage = (event) => {
            if (event.key === 'campusfix_shared_data_signal') refreshEvidence();
        };
        const onVisibility = () => {
            if (document.visibilityState === 'visible') refreshEvidence();
        };
    ...
    const load = async ({ quiet = false } = {}) => {
        ...
        const evidenceWorkOrder = orderedWorkOrders.find((item) => (item.completion_photo_urls || []).length > 0)
            || orderedWorkOrders.find((item) => (item.before_photo_urls || []).length > 0)
            || orderedWorkOrders[0]
            || null;
    ...
    if (role === 'admin') {
        ...
        return (<div className="admin-screen admin-complaint-detail-screen">
          ...
          <section className="admin-detail-section"><h2>Complaint Media</h2>{photos.length ? <div className="admin-detail-photos">{photos.map((url, i) => <a key={`${url}-${i}`} href={url} target="_blank" rel="noreferrer"><img src={url} alt={`Complaint photo ${i + 1}`} onError={onImageError}/></a>)}</div> : <div className="admin-detail-no-photos">No complaint photos uploaded.</div>}{(complaint.video_urls || []).length > 0 && <div className="admin-detail-videos">{(complaint.video_urls || []).map((url, i) => <video key={`${url}-${i}`} src={url} controls preload="metadata" aria-label={`Complaint video ${i + 1}`}/>)}</div>}</section>

          <section className="admin-detail-section admin-work-evidence-card">
            <h2>Work Evidence</h2>
            <div className="admin-work-evidence-grid">
              <div className="admin-work-evidence-column">
                <span className="admin-work-evidence-label">BEFORE</span>
                {(workOrder?.before_photo_urls || []).length > 0 ? <div className="admin-work-evidence-photos">{(workOrder.before_photo_urls || []).map((url, i) => <a key={`before-${i}`} href={url} target="_blank" rel="noreferrer"><img src={url} alt={`Before work ${i + 1}`} onError={onImageError}/></a>)}</div> : <div className="admin-work-evidence-empty">Before photo not available yet.</div>}
              </div>
              <div className="admin-work-evidence-column">
                <span className="admin-work-evidence-label">AFTER</span>
                {(workOrder?.completion_photo_urls || []).length > 0 ? <div className="admin-work-evidence-photos">{(workOrder.completion_photo_urls || []).map((url, i) => <a key={`after-${i}`} href={url} target="_blank" rel="noreferrer"><img src={url} alt={`After work ${i + 1}`} onError={onImageError}/></a>)}</div> : <div className="admin-work-evidence-empty">{complaint.status === 'in_progress' ? 'After photo will appear after work is completed.' : 'After photo not available yet.'}</div>}
              </div>
            </div>
            {workOrder?.repair_notes && <div className="admin-work-completion-note"><span>Completion note</span><p>{workOrder.repair_notes}</p></div>}
          </section>

          <section className="admin-detail-section admin-detail-timeline-section"><h2>Timeline</h2><div className="admin-detail-timeline">{timelineRows.map((item, index) => { const cfg = STATUS_CONFIG[item.new_status] || STATUS_CONFIG.submitted; return <div key={item.id || index} className={index === timelineRows.length - 1 ? 'is-last' : ''}><i/><p><strong>{cfg.label}</strong><span>{item.created_at ? formatDate(item.created_at) : '—'}</span>{item.remarks && <small>{item.remarks}</small>}</p></div>; })}</div></section>

          <section className="admin-detail-section admin-student-rating-card">
            <h2>Student Work Rating</h2>
            {complaint.feedback_rating ? <div className="admin-student-rating-value"><Star size={20}/><strong>{Number(complaint.feedback_rating).toFixed(1)} / 5</strong>{complaint.feedback_comment && <p>{complaint.feedback_comment}</p>}</div> : <div className="admin-student-rating-empty"><Star size={20}/><div><strong>No rating yet</strong><span>The student's rating will appear here after the completed complaint is reviewed.</span></div></div>}
          </section>
