...
import { ClipboardList, CheckCircle2, Clock, AlertTriangle, PlusCircle, ArrowRight, Wrench, Users, UserCog, BarChart3, Star, Bell, MessageSquareText, Wifi, Utensils, BookOpen, Megaphone, TrendingUp } from 'lucide-react';
...
export function DashboardScreen({ onNavigate, onOpenComplaint }) {
    ...
    if (isAdmin) {
        ...
        return (<div className="admin-dashboard-page">
          ...
          <div className="admin-reference-stats">
            <AdminReferenceStat icon={ClipboardList} tone="blue" label="Total Complaints" value={stats.total} meta={`${stats.open} currently active`}/>
            <AdminReferenceStat icon={AlertTriangle} tone="green" label="Open" value={stats.open} meta={`${stats.unassigned} awaiting assignment`}/>
            <AdminReferenceStat icon={Clock} tone="cyan" label="In Progress" value={inProgress} meta={`Across ${Math.max(activeStaff, 1)} active staff`}/>
            <AdminReferenceStat icon={Star} tone="amber" label="Avg. Rating" value={averageRating === '—' ? '—' : `${averageRating} /5`} meta={`From ${ratedComplaints.length} feedbacks`}/>
          </div>

          <div className="admin-quick-grid">
            <button onClick={() => onNavigate('assign')}><span><UserCog/></span><b>Assign Work</b><small>Allocate pending tasks</small></button>
            <button onClick={() => onNavigate('work-orders')}><span><ClipboardList/></span><b>Work Orders</b><small>Track maintenance work</small></button>
            <button onClick={() => onNavigate('reports')}><span><BarChart3/></span><b>View Reports</b><small>Performance analytics</small></button>
            <button onClick={() => onNavigate('users')}><span><Users/></span><b>User Management</b><small>Manage staff access</small></button>
          </div>

          <div className="admin-dashboard-panels">
            <Card className="admin-reference-panel">
              <div className="admin-reference-panel-head">
                <div><h3>Recent Activity</h3><p>Latest maintenance updates</p></div>
                <button onClick={() => onNavigate('work-orders')}>View all <ArrowRight className="w-3.5 h-3.5"/></button>
              </div>
            ...
            <Card className="admin-reference-panel admin-service-panel">
              ...
              <div className="admin-service-health">
                ...
                <div className="admin-service-legend">
                  <div><i className="resolved"/><span>Closed</span><b>{resolvedRate}%</b></div>
    ...
    if (isStudent) {
        return <StudentDashboard complaints={complaints} firstName={firstName} onNavigate={onNavigate} onOpenComplaint={onOpenComplaint}/>;
    }
...
function StudentDashboard({ complaints, firstName, onNavigate, onOpenComplaint }) {
    const rated = complaints.filter((complaint) => Number(complaint.feedback_rating) > 0);
    const average = rated.length
        ? rated.reduce((sum, complaint) => sum + Number(complaint.feedback_rating || 0), 0) / rated.length
        : 0;
    const recent = complaints.slice(0, 3);

    const iconFor = (complaint) => {
        const text = `${complaint?.title || ''} ${complaint?.complaint_categories?.name || ''} ${complaint?.category_name || ''}`.toLowerCase();
        if (text.includes('wifi') || text.includes('wi-fi') || text.includes('network')) return Wifi;
        if (text.includes('food') || text.includes('mess') || text.includes('canteen')) return Utensils;
        if (text.includes('library') || text.includes('seat') || text.includes('book')) return BookOpen;
        return Wrench;
    };

    const statusFor = (status) => {
        if (['resolved', 'closed'].includes(status)) return { label: 'Closed', tone: 'closed' };
        if (['assigned', 'in_progress'].includes(status)) return { label: 'In Progress', tone: 'progress' };
        return { label: 'Pending', tone: 'pending' };
    };

    return (
      <div className="student-screen student-home-screen">
        <div className="student-home-topline">
          <div>
            <span className="student-home-kicker">Home</span>
            <h1>Hi, {firstName} <span aria-hidden="true">👋</span></h1>
          ...
          <button type="button" className="student-round-icon" onClick={() => onNavigate('notifications')} aria-label="Open notifications">
            <Bell size={24}/><span className="student-notification-dot"/>
          </button>
        </div>

        <div className="student-rating-hero">
          <div className="student-rating-star"><Star size={30} fill="currentColor"/></div>
          <div className="student-rating-copy">
            <span>Overall Rating</span>
            <strong>{average ? average.toFixed(1) : '—'}<small>/5</small></strong>
            <p>Based on {rated.length} feedback{rated.length === 1 ? '' : 's'}</p>
          </div>
          <div className="student-rating-chart"><TrendingUp size={40}/></div>
        </div>

        <section className="student-home-section">
          <h2>Quick Actions</h2>
          <div className="student-quick-grid">
            <button type="button" onClick={() => onNavigate('raise')}><MessageSquareText/><span>Submit<br/>Feedback</span></button>
            <button type="button" onClick={() => onNavigate('my-complaints')}><ClipboardList/><span>Track<br/>Status</span></button>
            <button type="button" onClick={() => onNavigate('feedback')}><BarChart3/><span>View<br/>Ratings</span></button>
          </div>
        </section>

        <section className="student-home-section">
          <div className="student-section-title-row"><h2>Recent Feedbacks</h2><button type="button" onClick={() => onNavigate('my-complaints')}>View All <ArrowRight size={16}/></button></div>
          <div className="student-recent-list">
            {!recent.length ? (
              <div className="student-empty-card"><ClipboardList size={28}/><strong>No feedback yet</strong><span>Your submitted complaints will appear here.</span></div>
            ) : recent.map((complaint) => {
                const Icon = iconFor(complaint);
                const status = statusFor(complaint.status);
                const rating = Number(complaint.feedback_rating || 0);
                return (
                  <button type="button" key={complaint.id} onClick={() => onOpenComplaint(complaint.id)} className="student-recent-card">
                    <span className="student-recent-icon"><Icon size={24}/></span>
                    <span className="student-recent-copy">
                      <strong>{complaint.title}</strong>
                      {rating > 0 && <span className="student-stars">{'★'.repeat(Math.max(1, Math.min(5, Math.round(rating))))}<b>{rating.toFixed(1)}</b></span>}
                      <small>Submitted on {new Intl.DateTimeFormat('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }).format(new Date(complaint.created_at))}</small>
                    </span>
        ...
        <button type="button" className="student-info-banner" onClick={() => onNavigate('notifications')}>
          <span><Megaphone size={25}/></span>
          <b>Campus updates & maintenance alerts</b>
          <ArrowRight size={22}/>
        </button>
      </div>
    );
