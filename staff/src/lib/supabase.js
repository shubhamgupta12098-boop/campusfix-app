...
import { announceNotificationChange } from '@/lib/notificationBus';
    ...
    async execute() {
        try {
            ...
            if (this.action === 'insert') {
                ...
                // Complaint evidence is mandatory at the data layer too. This keeps
                // complaints from being created even if a caller bypasses the form UI.
                if (this.table === 'complaints') {
                    const missingEvidence = values.some((value) => {
                    ...
                    if (missingEvidence) {
                        return {
                            data: null,
                            error: { message: 'At least one photo is required to raise a complaint.' },
                        };
                    }
                }
                ...
                const made = values.map((value) => ({
                    ...(this.table === 'notifications' ? { is_read: false } : {}),
                    ...value,
                ...
                if (this.table === 'notifications') announceNotificationChange();
