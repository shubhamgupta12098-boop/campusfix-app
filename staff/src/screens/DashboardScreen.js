...
export function DashboardScreen({ onNavigate, onOpenComplaint, unreadNotifications = 0 }) {
    ...
    if (isStaff) {
        return <StaffDashboard complaints={complaints} firstName={firstName} onNavigate={onNavigate} onOpenComplaint={onOpenComplaint} unreadNotifications={unreadNotifications}/>;
    }
...
function StaffDashboard({ complaints, firstName, onNavigate, onOpenComplaint, unreadNotifications = 0 }) {
    const activeJobs = complaints.filter((c) => ['assigned', 'in_progress', 'waiting_approval'].includes(c.status));
    const assignedToday = activeJobs.filter((c) => {
        if (!c.assigned_at && !c.created_at) return false;
        const d = new Date(c.assigned_at || c.created_at);
        const n = new Date();
        return d.toDateString() === n.toDateString();
    }).length;
    const urgent = activeJobs.filter((c) => ['high', 'emergency'].includes(c.priority)).length;
    const completed = complaints.filter((c) => ['resolved', 'closed'].includes(c.status)).length;
    const pending = activeJobs.filter((c) => c.status === 'waiting_approval').length;
    const recent = complaints.slice(0, 5);

    const iconFor = (complaint) => {
        const text = `${complaint?.title || ''} ${complaint?.category_name || ''} ${complaint?.complaint_categories?.name || ''}`.toLowerCase();
        if (text.includes('wifi') || text.includes('wi-fi') || text.includes('network')) return Wifi;
        if (text.includes('food') || text.includes('mess')) return Utensils;
    ...
    return (
      <div className="staff-screen staff-home-screen">
        <div className="staff-screen-header">
          <div>
            <span className="staff-screen-kicker">Home</span>
            <h1>Welcome, {firstName}!</h1>
            <p>Here’s your maintenance overview.</p>
          </div>
          <div className="staff-header-actions">
            <button type="button" onClick={() => onNavigate('notifications')} aria-label="Notifications" className="staff-icon-button">
              <Bell size={22}/>{unreadNotifications > 0 && <span className="staff-alert-count">{unreadNotifications > 9 ? '9+' : unreadNotifications}</span>}
            </button>
          </div>
        </div>
        ...
        <section className="staff-welcome-card">
          <div>
            ...
            <strong>{activeJobs.length} active jobs</strong>
            <span>{urgent ? `${urgent} urgent task${urgent === 1 ? '' : 's'} need attention` : 'All assigned tasks are on track'}</span>
          </div>
          <div className="staff-avatar-badge">{firstName?.slice(0, 1)?.toUpperCase() || 'S'}</div>
        </section>

        <section className="staff-stat-grid">
          <button type="button" onClick={() => onNavigate('technician-jobs')} className="staff-stat-card blue">
            <span><ClipboardList size={22}/></span><small>Assigned Today</small><strong>{assignedToday}</strong><em>Jobs</em>
          </button>
          <button type="button" onClick={() => onNavigate('technician-jobs')} className="staff-stat-card red">
            <span><AlertTriangle size={22}/></span><small>Urgent Complaints</small><strong>{urgent}</strong><em>Jobs</em>
          </button>
          <button type="button" onClick={() => onNavigate('technician-jobs')} className="staff-stat-card green">
            <span><CheckCircle2 size={22}/></span><small>Completed Jobs</small><strong>{completed}</strong><em>Total</em>
          </button>
          <button type="button" onClick={() => onNavigate('technician-jobs')} className="staff-stat-card amber">
            <span><Clock size={22}/></span><small>Pending Approval</small><strong>{pending}</strong><em>Jobs</em>
          </button>
        </section>

        <section className="staff-section">
          <div className="staff-section-head"><h2>Recent Assigned Tasks</h2><button type="button" onClick={() => onNavigate('technician-jobs')}>View All <ArrowRight size={15}/></button></div>
            ...
            {!recent.length ? <div className="staff-empty">No assigned jobs yet.</div> : recent.map((complaint) => {
                ...
                return (
                  <button key={complaint.id} type="button" onClick={() => onOpenComplaint(complaint.id)} className="staff-task-card">
                    <span className="staff-task-icon"><Icon size={21}/></span>
                    <span className="staff-task-copy"><strong>{complaint.title}</strong><small>{location}</small></span>
                    <span className={`staff-status-chip ${tone}`}>{label}</span>
                    <ChevronRight size={16} className="staff-task-chevron"/>
                  </button>
                );
            })}
          </div>
