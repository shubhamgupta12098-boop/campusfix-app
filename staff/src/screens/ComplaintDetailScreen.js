...
export function ComplaintDetailScreen({ complaintId, onBack, onNavigate, unreadNotifications = 0 }) {
    ...
    if (role === 'staff') {
        return (<StaffComplaintDetailView
          complaint={complaint}
          history={history}
          workOrder={workOrder}
          onBack={onBack}
    ...
    if (role === 'admin') {
        const photos = (complaint.photo_urls || []).slice(0, 3);
        const timelineRows = history.length ? history : [
            { id: 'reported', new_status: 'submitted', created_at: complaint.created_at },
            ...(complaint.assigned_at ? [{ id: 'assigned', new_status: 'assigned', created_at: complaint.assigned_at }] : []),
            ...(complaint.resolved_at ? [{ id: 'resolved', new_status: 'closed', created_at: complaint.resolved_at }] : []),
        ];
        const statusLabel = sc?.label || displayStatus.replaceAll('_', ' ');
        const priorityLabel = pc?.label || complaint.priority || 'Medium';
        const selectedStaff = technicians.find((item) => item.id === selectedTech) || null;
        const genuineDecision = complaint.status === 'submitted' ? 'pending' : complaint.status === 'rejected' ? 'no' : 'yes';
        return (<div className="admin-screen admin-complaint-detail-screen">
          <div className="admin-detail-identity">
            <span>{complaint.complaint_no || `#${complaint.id}`}</span>
            <h1>{complaint.title}</h1>
            <div className="admin-detail-chips"><b className="status"><CheckCircle2 size={16}/>{statusLabel}</b><b className="priority"><AlertCircle size={16}/>{priorityLabel}</b></div>
          </div>

          <div className="admin-detail-info-list">
            <div><MapPin/><span>Location</span><strong>{complaint.buildings?.name || complaint.location_description || 'Campus'}</strong></div>
            <div><User/><span>Reported by</span><strong>{complaint.profiles?.full_name || 'Campus User'}</strong></div>
            <div><Clock/><span>Date</span><strong>{formatDate(complaint.created_at)}</strong></div>
            <div><Wrench/><span>Assigned to</span><strong>{complaint.assigned_profile?.full_name ? `${complaint.assigned_profile.full_name} - ${complaint.assigned_profile.department || 'Maintenance'}` : 'Not assigned yet'}</strong></div>
          </div>

          <section className={`admin-genuine-card ${genuineDecision}`}>
            <div className="admin-genuine-heading">
              <span className="admin-genuine-icon"><ShieldCheck size={20}/></span>
              <div><h2>Genuine Complaint?</h2><p>Admin must verify this complaint before assigning it to staff.</p></div>
            </div>
            {complaint.status === 'submitted' ? <div className="admin-genuine-actions">
              <button type="button" className="no" disabled={reviewing} onClick={() => void reviewComplaint(false)}><Ban size={18}/>No</button>
              <button type="button" className="yes" disabled={reviewing} onClick={() => void reviewComplaint(true)}><CheckCircle2 size={18}/>{reviewing ? 'Saving…' : 'Yes'}</button>
            </div> : <div className={`admin-genuine-result ${genuineDecision}`}>
              {genuineDecision === 'yes' ? <><CheckCircle2 size={18}/><strong>Yes — Genuine Complaint</strong></> : <><Ban size={18}/><strong>No — Not Genuine</strong></>}
            </div>}
          </section>

          {['verified', 'assigned'].includes(complaint.status) && <section className="admin-staff-assignment-card">
            <div className="admin-staff-assignment-title"><Wrench size={19}/><div><h2>Assign Staff</h2><p>Choose who will handle this complaint.</p></div></div>
            {complaint.assigned_profile?.full_name && <div className="admin-current-staff"><span>Currently assigned</span><strong>{complaint.assigned_profile.full_name}</strong><small>{complaint.assigned_profile.department || 'Maintenance'}</small></div>}
            <label className="admin-staff-select-label">Staff member
            ...
            {selectedStaff && <div className="admin-selected-staff">
              <span className="admin-selected-staff-avatar">{selectedStaff.full_name?.split(' ').map((part) => part[0]).slice(0,2).join('').toUpperCase() || 'ST'}</span>
              <div><span>Assigning to</span><strong>{selectedStaff.full_name}</strong><small>{selectedStaff.department || 'Maintenance'} · {selectedStaff.technician?.availability_status === 'busy' ? 'Busy' : 'Available'}</small></div>
            </div>}
            <button type="button" className="admin-assign-confirm" disabled={!selectedTech || assigningStaff} onClick={() => void assignReviewedComplaint()}><Send size={18}/>{assigningStaff ? 'Assigning…' : complaint.status === 'assigned' ? 'Reassign to Selected Staff' : 'Assign to Selected Staff'}</button>
          </section>}

          <section className="admin-detail-section"><h2>Description</h2><p>{complaint.description || 'No description provided.'}</p></section>

          <section className="admin-detail-section"><h2>Photos</h2>{photos.length ? <div className="admin-detail-photos">{photos.map((url, i) => <a key={`${url}-${i}`} href={url} target="_blank" rel="noreferrer"><img src={url} alt={`Complaint photo ${i + 1}`} onError={onImageError}/></a>)}</div> : <div className="admin-detail-no-photos">No complaint photos uploaded.</div>}</section>

          <section className="admin-detail-section admin-detail-timeline-section"><h2>Timeline</h2><div className="admin-detail-timeline">{timelineRows.map((item, index) => { const cfg = STATUS_CONFIG[item.new_status] || STATUS_CONFIG.submitted; return <div key={item.id || index} className={index === timelineRows.length - 1 ? 'is-last' : ''}><i/><p><strong>{cfg.label}</strong><span>{item.created_at ? formatDate(item.created_at) : '—'}</span></p></div>; })}</div></section>

          {adminMessage && <div className="admin-inline-message">{adminMessage}</div>}
          {canClose && <div className="admin-detail-actions admin-detail-actions-single"><button type="button" className="primary" disabled={closing} onClick={() => void closeComplaint()}><LockKeyhole size={20}/>{closing ? 'Closing…' : 'Close Complaint'}</button></div>}
        </div>);
    }
...
function StaffComplaintDetailView({ complaint, history, workOrder, onBack, onNavigate, unreadNotifications = 0 }) {
    const statusKey = complaint.status === 'resolved' ? 'closed' : complaint.status;
    const status = STATUS_CONFIG[statusKey] || STATUS_CONFIG.submitted;
    ...
    const reporter = complaint.profiles?.full_name || 'Student';
    ...
    const statusTone = statusKey === 'closed'
        ? 'closed'
        : ['in_progress', 'assigned', 'verified', 'waiting_approval'].includes(statusKey)
            ? 'progress'
            : statusKey === 'rejected'
                ? 'rejected'
                : 'pending';
    const priorityTone = complaint.priority === 'high' || complaint.priority === 'emergency'
        ...
        : complaint.priority === 'medium'
            ? 'medium'
            : 'low';
    const feedbackRating = Number(complaint.feedback_rating || 0);
    const canUpdateStatus = ['assigned', 'in_progress'].includes(complaint.status);
    const beforePhotos = workOrder?.before_photo_urls || [];
    const afterPhotos = workOrder?.completion_photo_urls || [];

    return (<div className="student-screen student-complaint-detail-screen staff-complaint-detail-screen">
      ...
      <section className="student-detail-hero">
        ...
        <div className="student-detail-badges">
          <span className={`student-detail-status ${statusTone}`}>
            {statusKey === 'closed' ? <Check size={15}/> : <Clock size={15}/>} {status.label}
          </span>
      ...
      {(photos.length > 0 || videos.length > 0) && (<StudentDetailSection title="Complaint Photos">
      ...
      {(beforePhotos.length > 0 || afterPhotos.length > 0 || workOrder?.repair_notes) && (<StudentDetailSection title="Work Evidence">
          {(beforePhotos.length > 0 || afterPhotos.length > 0) && <div className="staff-detail-evidence-grid">
            <div>
              <span className="staff-detail-evidence-label">Before</span>
              <div className="staff-detail-evidence-media">
                {beforePhotos.map((url, index) => <a key={`before-${index}`} href={url} target="_blank" rel="noreferrer"><img src={url} alt={`Before work ${index + 1}`} onError={onImageError}/></a>)}
              </div>
            </div>
            <div>
              <span className="staff-detail-evidence-label">After</span>
              <div className="staff-detail-evidence-media">
                {afterPhotos.map((url, index) => <a key={`after-${index}`} href={url} target="_blank" rel="noreferrer"><img src={url} alt={`After work ${index + 1}`} onError={onImageError}/></a>)}
              </div>
            </div>
          </div>}
      ...
      <StudentDetailSection title="Student Work Rating">
        <div className={`staff-detail-rating-card ${feedbackRating > 0 ? 'has-rating' : ''}`}>
          {feedbackRating > 0 ? (<>
              <div className="staff-detail-rating-top">
                <div>
                  <span className="staff-detail-rating-score">{feedbackRating.toFixed(1)}</span>
                  <small>/5</small>
                </div>
                <div className="staff-detail-rating-stars" aria-label={`${feedbackRating} out of 5 stars`}>
                  {[1, 2, 3, 4, 5].map((number) => <Star key={number} size={22} className={number <= Math.round(feedbackRating) ? 'is-active' : ''} fill={number <= Math.round(feedbackRating) ? 'currentColor' : 'none'}/>) }
                </div>
              </div>
              <p>{complaint.feedback_comment || 'Student submitted a rating without a written comment.'}</p>
              {complaint.feedback_submitted_at && <span className="staff-detail-rating-date">Rated on {formatDate(complaint.feedback_submitted_at)}</span>}
            </>) : (<div className="staff-detail-rating-empty">
              <Star size={25}/>
              <div><strong>No rating yet</strong><span>The student's rating will appear here after the completed complaint is reviewed.</span></div>
            </div>)}
        </div>
      </StudentDetailSection>

      <div className="student-detail-actions staff-detail-actions">
        {canUpdateStatus ? <button type="button" onClick={() => onNavigate?.('technician-jobs')} className="student-detail-primary-action">
            <Pencil size={20}/> Update Status
          </button> : complaint.status === 'waiting_approval' ? <div className="student-detail-note"><Clock size={16}/><span>Work submitted. Waiting for admin approval.</span></div> : statusKey === 'closed' ? <div className="student-detail-note success"><CheckCircle2 size={16}/><span>This job is closed. You can still view the student's work rating above.</span></div> : null}
      </div>
    </div>);
}
