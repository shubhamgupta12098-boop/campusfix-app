...
import { Bell, CheckCircle2, ClipboardCheck, MessageSquare, Star, UserRound, Wrench, AlertCircle, Clock, Megaphone, CheckCheck } from 'lucide-react';
...
export function NotificationsScreen({ onOpenComplaint }) {
  ...
  const markAllRead = async () => {
    const ids = notifications.filter((item) => item?.is_read !== true).map((item) => item.id).filter(Boolean);
    if (ids.length) await localData.from('notifications').update({ is_read: true }).in('id', ids);
    await load();
  };
  ...
  if (isStaff) {
    return <div className="staff-screen staff-notifications-screen">
      <div className="staff-notification-heading">
        <div><span>Alerts</span><h1>Notifications</h1><p>{unread > 0 ? `${unread} unread update${unread === 1 ? '' : 's'}` : 'You are all caught up'}</p></div>
        {unread > 0 && <button type="button" onClick={() => void markAllRead()}><CheckCheck size={17}/>Read all</button>}
      </div>
      {notifications.length === 0 ? <div className="staff-notification-empty"><Bell size={29}/><strong>No notifications</strong><span>New job assignments and approval updates will appear here.</span></div> : <div className="staff-notification-list">
        {notifications.map((n) => {
          const Icon = TYPE_ICONS[n.type] || Megaphone;
          const unreadItem = n?.is_read !== true;
          return <button type="button" key={n.id} onClick={() => void openNotification(n)} className={`staff-notification-card ${unreadItem ? 'is-unread' : 'is-read'}`}>
            <span className="staff-notification-icon"><Icon size={21}/></span>
            <span className="staff-notification-copy"><strong>{n.title || 'CampusFix update'}</strong><p>{n.message || 'Open to view the related job.'}</p><small>{timeAgo(n.created_at)}</small></span>
            {unreadItem && <i className="staff-notification-unread"/>}
          </button>;
        })}
      </div>}
    </div>;
