    ...
    return (<div className="max-w-5xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-5">
        <div className="flex-1 relative">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by title or complaint ID…" className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 text-sm text-slate-900 bg-white"/>
        </div>
        <div className="flex items-center gap-2 overflow-x-auto pb-1">
          <Filter className="w-4 h-4 text-slate-400 flex-shrink-0"/>
          {['all', 'submitted', 'verified', 'assigned', 'in_progress', 'closed'].map((s) => (<button key={s} onClick={() => setStatusFilter(s)} className={`px-3 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap transition-colors ${statusFilter === s ? 'bg-blue-600 text-white' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
              {s === 'all' ? 'All' : STATUS_CONFIG[s].label}
            </button>))}
        </div>
      </div>

      {loadError && (<div className="mb-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
      ...
      {filtered.length === 0 ? (<Card className="p-0">
          ...
          {filtered.map((c) => {
                ...
                return (<button key={c.id} onClick={() => onOpenComplaint(c.id)} className="w-full text-left">
