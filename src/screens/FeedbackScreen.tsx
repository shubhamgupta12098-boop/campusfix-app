...
export function FeedbackScreen({ onOpenComplaint }: { onOpenComplaint: (id: string) => void }) {
  ...
  return <div className="max-w-5xl mx-auto space-y-5">
    ...
    {!rows.length ? <Card className="p-8"><EmptyState icon={MessageSquare} title="No feedback yet" description={isAdmin ? 'Student ratings will appear here.' : 'You can rate a complaint here after it has been resolved.'}/></Card> :
