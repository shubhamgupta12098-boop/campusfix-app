...
import { GraduationCap, BadgeCheck, Wrench, ShieldCheck, Crown, Mail, Lock, User, Phone, Building2, Home, DoorOpen } from 'lucide-react';
...
const ROLES: { value: UserRole; label: string; icon: typeof GraduationCap; color: string }[] = [
  ...
  { value: 'faculty', label: 'Faculty / Staff', icon: BadgeCheck, color: 'cyan' },
  { value: 'technician', label: 'Technician', icon: Wrench, color: 'amber' },
  { value: 'supervisor', label: 'Supervisor', icon: ShieldCheck, color: 'violet' },
  { value: 'admin', label: 'Admin', icon: Crown, color: 'rose' },
];
...
export function AuthScreen() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'login') {
      ...
      await signUp({ email, password, fullName, role, collegeId, department, hostel, block, room, phone });
    }
    setSubmitting(false);
  };
