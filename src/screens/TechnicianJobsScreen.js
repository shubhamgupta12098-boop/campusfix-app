...
export function TechnicianJobsScreen({ onOpenComplaint }) {
    ...
    const openCompleteWork = async (c) => {
        setWorkError('');
        setRepairNotes('');
        setCompletionPhotos([]);
        setBeforePhotos([]);
    ...
    const handlePhotoUpload = async (e, kind) => {
        ...
        try {
            ...
            if (kind === 'before')
                setBeforePhotos((prev) => [...prev, ...urls].slice(0, 5));
            else
                setCompletionPhotos((prev) => [...prev, ...urls].slice(0, 5));
        }
        catch (error) {
    ...
    const confirmStartWork = async () => {
        ...
        if (beforePhotos.length < 1) {
            setWorkError('Please add at least one before-repair photo showing the issue.');
            return;
        }
        setUploading(true);
        try {
            const updates = { status: 'in_progress', updated_at: new Date().toISOString() };
            if (!startModal.assigned_at)
    ...
    const completeJob = async () => {
        ...
        if (completionPhotos.length < 1) {
            setWorkError('At least one after-repair photo is required.');
            return;
        }
        if (!repairNotes.trim()) {
            setWorkError('Completion remarks are required.');
            return;
        }
    ...
    return (<div className="max-w-4xl mx-auto">
      ...
      {workError && !workModal && (<Card className="p-4 mb-4 border border-red-200 bg-red-50">
      ...
      {list.length === 0 ? (<Card className="p-0">
          ...
          {list.map((c) => {
                ...
                <div className="flex gap-2 mt-3 pt-3 border-t border-slate-100">
                  {activeTab === 'active' && c.status === 'assigned' && (<button onClick={() => openStartWork(c)} className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg bg-blue-600 text-white text-xs font-semibold hover:bg-blue-700">
                      <Play className="w-3.5 h-3.5"/> Start Work
                    </button>)}
                  {activeTab === 'active' && c.status === 'in_progress' && (<button onClick={() => void openCompleteWork(c)} className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg bg-emerald-600 text-white text-xs font-semibold hover:bg-emerald-700">
      ...
      {startModal && (<div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => { setStartModal(null); setWorkError(''); }}>
            ...
            <div onClick={(e) => e.stopPropagation()}>
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 mb-4 space-y-2">
                ...
                <div className="flex flex-wrap gap-3 text-xs text-slate-600 pt-1">
                  {startModal.complaint_categories?.name && (<span className="flex items-center gap-1"><Tag className="w-3.5 h-3.5 text-slate-400"/>{startModal.complaint_categories.name}</span>)}
                  {(startModal.buildings?.name || startModal.location_description) && (<span className="flex items-center gap-1"><MapPin className="w-3.5 h-3.5 text-slate-400"/>{startModal.buildings?.name}{startModal.location_description ? ` · ${startModal.location_description}` : ''}{startModal.floor ? ` · Floor ${startModal.floor}` : ''}</span>)}
                  {startModal.expected_completion && (<span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5 text-slate-400"/>Due {formatDate(startModal.expected_completion)}</span>)}
                </div>
      ...
      {workModal && (<div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => { setWorkModal(null); setWorkError(''); }}>
          <Card className="p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between mb-4">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  {loadingWorkOrder ? (<p className="text-xs text-slate-400">Loading…</p>) : beforePhotos.length > 0 ? (<div className="flex flex-wrap gap-2">{beforePhotos.map((url, i) => <a key={url} href={url} target="_blank" rel="noreferrer"><img src={url} alt={`Before repair ${i + 1}`} className="w-16 h-16 object-cover rounded-lg border border-slate-200" onError={onImageError}/></a>)}</div>) : (<p className="text-xs text-slate-400">No before photo was captured for this job.</p>)}
                </div>
                <div>
