...
export function WorkOrdersScreen({ onOpenComplaint }) {
    ...
    return (<div className="max-w-5xl mx-auto">
      ...
      {loadError && (<Card className="p-4 mb-4 border border-red-200 bg-red-50">
      ...
      {!loadError && orders.length === 0 ? (<Card className="p-0"><EmptyState icon={ClipboardList} title="No work orders" description="Staff starts a work order after an assigned complaint is opened."/></Card>) : (<div className="space-y-4">
          {orders.map((wo) => {
                const complaint = wo.complaints;
                return (<Card key={wo.id} className="p-5">
                <div className="flex items-start gap-3">
                  {complaint?.photo_urls?.[0] ? (<button onClick={() => complaint?.id && onOpenComplaint(complaint.id)} className="w-16 h-16 rounded-xl overflow-hidden flex-shrink-0 border border-slate-200">
                      <img src={complaint.photo_urls[0]} alt="Complaint" className="w-full h-full object-cover" onError={onImageError}/>
                    </button>) : (<div className="w-12 h-12 rounded-xl bg-slate-100 flex items-center justify-center flex-shrink-0">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {complaint?.id && (<button onClick={() => onOpenComplaint(complaint.id)} className="mt-4 inline-flex items-center gap-1.5 px-4 py-2 rounded-lg bg-blue-600 text-white text-xs font-semibold hover:bg-blue-700">
...
function Evidence({ title, urls, empty }) {
    return (<div className="rounded-xl border border-slate-200 p-3">
      ...
      {urls.length ? (<div className="grid grid-cols-2 gap-2">
          {urls.slice(0, 4).map((url, i) => (<a key={`${url}-${i}`} href={url} target="_blank" rel="noreferrer" className="aspect-square rounded-lg overflow-hidden border border-slate-200">
              <img src={url} alt={`${title} ${i + 1}`} className="w-full h-full object-cover" onError={onImageError}/>
            </a>))}
        </div>) : <p className="text-xs text-slate-400 py-5 text-center">{empty}</p>}
    </div>);
}
