...
import { Download, TrendingUp, Clock, AlertTriangle, FileBarChart } from 'lucide-react';
export function ReportsScreen() {
    ...
    const load = async () => {
        ...
        if (profile?.role === 'staff')
            complaintQuery = complaintQuery.eq('assigned_to', profile.id);
    ...
    const exportCSV = async () => {
        ...
        setExporting(true);
        try {
            await downloadTextFile(`complaints-report-${new Date().toISOString().slice(0, 10)}.csv`, csv, 'text/csv');
        }
        catch (error) {
    ...
    return (<div className="max-w-6xl mx-auto">
      <PageHeader title={profile?.role === 'staff' ? 'My Reports' : 'Reports & Analytics'} subtitle={profile?.role === 'staff' ? 'Only your assigned and completed work is shown' : 'Insights into campus maintenance performance'} action={<button onClick={exportCSV} disabled={exporting} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-sm font-semibold shadow-lg transition-all disabled:opacity-60">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="p-5">
          ...
          <div className="space-y-3">
            {stats.byCategory.map((cat) => (<div key={cat.id}>
        ...
        <Card className="p-5">
          ...
          {stats.byBuilding.length === 0 ? <p className="text-sm text-slate-400">No data</p> : (<div className="space-y-3">
        ...
        {profile?.role === 'admin' && <Card className="p-5 lg:col-span-2">
          ...
          {stats.byTechnician.length === 0 ? <p className="text-sm text-slate-400">No assigned complaints yet</p> : (<div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {stats.byTechnician.map((t) => {
                    ...
                    return (<tr key={t.id} className="border-b border-slate-50 last:border-0">
